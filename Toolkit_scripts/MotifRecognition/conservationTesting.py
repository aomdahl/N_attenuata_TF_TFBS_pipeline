...
def findOrtholog(geneID, orthologMapping, geneMap = None, rand = False):
	...
	if rand is True:
		...
		if (geneMap is not None) and (geneID in orthologMapping):  #Need to make sure its actually in there.
			setGeneOrtholog(geneID, orthologMapping, geneMap,orthologMapping[geneID])
...
def setGeneOrtholog(geneID, orthologMapping, geneMap, ortholog):
...
def loadOrthologMap(orthoPlant):
	orthoFilePath = GU.ortho_DB_paths[orthoPlant]
	orthologMap = {}
	...
	print orthoFilePath, "successfully loaded"
	raw_input(orthoFilePath)
	return orthologMap
...
def updateConservedMotifMap(currentGene, align_1,  align_2, conservedMotifs, noOverlapsList = None, updateMasterList = True):
	...
	if checkConservationOnAlignment(currentGene, motifOverlaps, noOverlapsList):
...
def checkConservationOnAlignment(currentGene, intersectOutput, noMotifOverlaps):
...
def runYASSAlignment(geneKey, geneOrtholog, NIATPromotersPath, orthologPromotersPath, folderName = None):
	...
	orthologPromoterNewFile= createLocalPromoterCopies(geneOrtholog, (orthologPromotersPath  +args.kb), folderName)
	os.chdir(folderName)
	os.system("yass ./" + genePromoterNewFile + " ./" + orthologPromoterNewFile + " -o " + gene_orthoName + "_alignment.bed -d 4")

def buildYASSAlignmentScript(geneKey, geneOrtholog, NIATPromotersPath, orthologPromotersPath, scriptPath, folderName = None):
	yassScript = open(scriptPath, 'a')
	gene_orthoName = geneKey + '_' + geneOrtholog  #A combined name for file writing
	if folderName is None:
		folderName = gene_orthoName
	makeNewDirectory(folderName)  #Does this securely, so don't script this in?
	genePromoterNewFile = createLocalPromoterCopies(geneKey, NIATPromotersPath, folderName, yassScript)
	
	orthologPromoterNewFile= createLocalPromoterCopies(geneOrtholog, (orthologPromotersPath + args.kb), folderName, yassScript)
	yassScript.write("cd " + folderName + '\n')
	#print "yass ./" + genePromoterNewFile + " ./" + orthologPromoterNewFile + " -o " + gene_orthoName + "_alignment.bed -d 4"
	yassScript.write("yass ./" + genePromoterNewFile + " ./" + orthologPromoterNewFile + " -o " + gene_orthoName + "_alignment.bed -d 4\n")
	yassScript.write("cd..\n")
	return folderName
...
#@param yassScript to write the copy command to
#@return the new file Name
def createLocalPromoterCopiesScript(geneID, promoterDBPath, newDirectory, yassScript):
	...
	if not os.path.isfile(newDirectory + "/"+ promoterNewFile):
		yassScript.write("cp " + promoterDBPath + '/' + promoterSourceFile + " ./" + newDirectory + "/"+ promoterNewFile + '\n')
...
def createLocalPromoterCopies(geneID, promoterDBPath, newDirectory):
	...
	#Check to see if its there already; if so, don't bother copying it.
	if not os.path.isfile(newDirectory + "/"+ promoterNewFile):
		raw_input("cp " + promoterDBPath + '/' + promoterSourceFile + " ./" + newDirectory + "/"+ promoterNewFile)
		os.system("cp " + promoterDBPath +'/' + promoterSourceFile + " ./" + newDirectory + "/"+ promoterNewFile)
	return promoterNewFile

#Is there a better place for this?
def findSignificanceOfConservationScores(currentGene, promoterDBPath, ortho_DB_path):
	...
	for putativeMotif in masterMotifList.motifMap:  #Being done the easy way- try our custom iterator sometime!
		...
		for i in range(0,args.reps):  
			...
			for gene in orthologousGenes:
				...
				randomOrtholog = findOrtholog(gene, Orthologmappinghere, rand = True)

				runYASSAlignment(gene, randomOrtholog, promoterDBPath, ortho_DB_path, folderName = gene)
				...
				conservationMap = updateConservedMotifMap(currentGene, gene_orthoName,newMotifAlignmentPath, conservationMap, updateMasterList = False)
...
#This function aligns the positive motifs to the specified ortholog and creates the respective directories
#@param ortho_db to compare with- use "All" for all 3
#@param noMotifOverlapsList: keep track of genes and their motifs lacking any overlaps in this particular list
def alignMotifs(ortho_db, NIAT_genes_path, NIAT_promoters_path):
	...
	#Repeat this for the organisms used
	if ortho_db == "All":
		orthologList = ["Tomato", "Arabidopsis", "Poplar"]
	else:
		orthologList = [ortho_db]

	for orthoPlant in orthologList:  #For each of the plants we want to examine the orthologs for
		print orthoPlant
		raw_input("Starting on...")
		orthologMapping = loadOrthologMap(orthoPlant)
		ortho_promoter_path = GU.ortho_promoter_paths[orthoPlant]
		for geneKey in motifGeneMap:
			#First: find the orthologs for each gene, update the ortholog in to the map.
			findOrtholog(geneKey, orthologMapping, geneMap = motifGeneMap)
			...
			if motifGeneMap[geneKey].ortholog is None:#This occurs if there is no ortholog.
				...
				if orthoPlant == "Arabidopsis":
					raw_input('no ortholog')
			else:  
				if orthoPlant == "Arabidopsis":
					raw_input(motifGeneMap[geneKey].ortholog)
					
				currentOrtholog = motifGeneMap[geneKey].ortholog
				runYASSAlignment(geneKey, currentOrtholog, NIAT_promoters_path, ortho_promoter_path)
				writeBedData(geneKey, geneMotifBED)
				gene_orthoName = geneKey + '_' + motifGeneMap[geneKey].ortholog
				totalConservation = updateConservedMotifMap(geneKey, gene_orthoName, geneKey, totalConservation, noOverlapsList = noMotifOverlapsList)
				os.chdir("../")

##NEED TO ACTUALL ACCESS THE RIGHT FOLDER
...
#ortho_DB_paths = {"Tomato":, "Arabidopsis":, "Poplar":} --> Stored in gene utils, with all the other paths and references.
...
if __name__ == '__main__':
	...
	parser.add_argument("-p", "--NIAT_promoters", default = "./SplitPromoters", help="Path to the NIAT promoter sequence database, all written to individual files")
	parser.add_argument("-g", "--NIAT_genes", help="Path to the .tsv file containing NIAT genes and their motifs.  Output of scanMotifGenomeWide.pl -bed")
	parser.add_argument("-db", "--ortho_db", choices = ["Tomato", "Arabidopsis", "Poplar", "All"], default = "All", help="Specify which ortholog you would like to use")
	...
	parser.add_argument("--pval", default = 1.0, type = float, help = "Specify the p-value threshold for writing out acceptable motifs.  Default is all")
	parser.add_argument("--alignMotifs", action="store_true", help = "Choose this option to align the motifs for the three different species")
	parser.add_argument("--kb", default = "1kb", help = "Specify the promtoer size you wish to examine.")
	...
	if args.alignMotifs:
		alignMotifs(args.ortho_db, NIAT_genes_path, NIAT_promoters_path)
