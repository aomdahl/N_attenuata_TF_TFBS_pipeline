...
combined_finalized_t1 <- read_tsv("D:/AshtonData/MPI_all_data/SummaryFiles_8-17/top_motifs_combined.tsv.txt") %>% group_by(pTF)%>% top_n(1, ValidationMatchScore) %>% ungroup() %>% group_by(pMotif) %>% top_n(1,`ConservationScore`) %>% ungroup() %>% filter(BLASTPercentIdentity >= 60) %>% select(-WGCNAModuleConnectivity) %>% group_by(pTF) %>% top_n(1, `TF-MotifMatchScore`) %>% ungroup() %T>% print()
  write_tsv(combined_finalized_t1,"D:/AshtonData/MPI_all_data/SummaryFiles_8-17/top_motifs_combined_FILTERED_alt.txt" )

combined_finalized_4_20 <- read_tsv("E:/AshtonData/MPI_all_data/SummaryFiles_8-17/top_motifs_combined.tsv.txt") %>% group_by(pMotif)%>% top_n(1, ValidationMatchScore) %>% ungroup() %>% group_by(pMotif) %>% top_n(1,`ConservationScore`) %>% ungroup()%>% group_by(pTF) %>% top_n(1, `TF-MotifMatchScore`) %>% ungroup() %>% group_by(pTF) %>% top_n(-1, `e-Value`) %>% filter(BLASTPercentIdentity >= 50) %>% select(-WGCNAModuleConnectivity) %>% ungroup() %>% group_by(pTF,Reference_seq) %>% top_n(1, ValidationMatchScore) %>% group_by(pTF) %>% top_n(1,ValidationMatchScore) %>% top_n(1,ConservationScore)

write_tsv(combined_finalized_4_20,"E:/AshtonData/MPI_all_data/SummaryFiles_8-17/top_motifs_combined_4_20.txt" )
